...
from .forms import ApprovalForm,UserForm, MyAuthForm, ApprovalForm2,ContactForm,ApprovalForm3
...
def Randomanimal():
	...
	animal_list = ['snake wildlife','Beetle', 'cat', 'dog', 'elephant', 'goat and cow','mammals','amphibians', 'leopard','antelop', 'monkeys','dolphins',
	...
	animal_list = random.choice(movie_list)
...
#TRiANING THE MODEL TO BE USED AFTER USER FILLS PREDICTION FORM

def prediction(unit):
    #unit serves as the user input 

	#Put in your storage details (we used AWS S3 Storage here)
    #the Training data is stored here
	try:
  
        s3 = boto3.resource('s3',
             aws_access_key_id='',
             aws_secret_access_key= '')
        #Access the datset the model is been trained on
        with open('', 'wb') as data:
            s3.Bucket("").download_fileobj("zoo.csv", data)

        with open('', 'rb') as data:
            df = pd.read_csv(data)
            
        #Drop unneccesary columns for training
        df=df.dropna()
       
        
        df4=df.drop("animal_name","catsize","class_type" axis=1)
		...
		#Assign classifier (Optional)
        mdl=clasifier
	    #Convert inputed user data to numpy array for prediction
        X=unit.to_numpy()
        print (X)
        y_pred=mdl.predict(X)
        newdf=pd.DataFrame(y_pred, columns=['class_type'])
		#Replace with target labels
